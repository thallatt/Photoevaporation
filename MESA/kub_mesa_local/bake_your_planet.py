...
maxage = 15.e9                          # final age in years
...
maxEntropy = 9.0				# entropy of inflated atmosphere in kb/baryon (init. thermal state; step iv in the paper)
...
ms = float(sys.argv[5]) # 				# star mass in msun
rs = 1.#				# star radius in rsun
Teff_star = 5780. #			# stellar Teff in K
orb_sep = (float(sys.argv[3])*(60*60*24)*(2.*np.pi)**-1.*(consts.G*ms*consts.Msun)**0.5)**(2./3.) / consts.au2cm #				# orbital sepration of the planet in AU
print('au='+str(orb_sep))
#BA= 0.0		                # planet Bond albedo
period_star = float(sys.argv[4])       # stellar rotation period
thermal_ = bool(str(sys.argv[6])=='True')         # with or without mass loss
...
escape_model = str(sys.argv[7])                # 'HD' stands for hydrodynamic HBA approximation (see Kubyshkina et al., 2018b)
...
do_put_in_core = bool(str(sys.argv[8])=='True') 		#put the core of the right size
do_relaxm = bool(str(sys.argv[9])=='True') 		#remove the atmosphere to the desired value
do_set_entropy = bool(str(sys.argv[10])=='True')		#set an artificial luminocity to inflate the planet
do_cool = bool(str(sys.argv[11])=='True') 			#remove an artificial luminocity and cool the planet for coolingTime
do_relax_irradiation =  bool(str(sys.argv[12])=='True')	#set stellar heating (Teq); 
do_evolve_planet = bool(str(sys.argv[13])=='True') 	#evolve!
##################################################################################################################################
...
evolvedmodel = "planet_7_evolved_"  + str(mp/mearth)[0:6] + "_ME_fat_" + str(fat_0)[0:6] +"_S"+ str(maxEntropy) +"_d_"+ str(round(orb_sep,3))+ "_" +str(ms)  + "Msun_" +str(period_star)+ "Pstar_JohnstoneLXUV+Lbol_"+str(escape_model)+"Escape_testing.mod"
evolvedmodel_thermal = "planet_7_evolved_"  + str(mp/mearth)[0:6] + "_ME_fat_" + str(fat_0)[0:6] +"_S"+ str(maxEntropy) +"_d_"+ str(round(orb_sep,3))+ "_" +str(ms)  + "Msun_" +str(period_star)+ "Pstar_Thermal_JohnstoneLXUV+Lbol_testing.mod"
#############################################################################################################################
#BEGIN

if (mp - mp * fat_0)/mearth > 22.:
...
if do_set_entropy:	
    ...
    print('current S='+str(currentropy))
    if currentropy<float(maxEntropy):
        ...
        run_time = inlist.remove_Lcore(mesh_delta_coeff, 5.e7, Lcore_low, inlist5,relaxedmodel,removemodel)
        success = True
...
if do_cool:
    Lcore_low = mcore*5e-8 #energy generation rate of present Earth; Doesn't matter, will be anyway overtaken by core model in ev.
    inlist5 = "tmp_inlists/inlist_5_removeLc_"  + str(mp/mearth)[0:6] + "_ME_fat" + str(fat_0)[0:6] +"_"+ str(maxEntropy)
    run_time = inlist.remove_Lcore(mesh_delta_coeff, 1.e7, Lcore_low, inlist5,entropymodel,removemodel)
    success = True
    if not os.path.exists(removemodel):
        success=False	
    k=open('LOGS/history_5_removeLc','r')
...
if do_relax_irradiation:    
    ...
    Lbol = np.interp(coolingTime, Etrack[:,0], Etrack[:,1]);# [log10(Lsun)] # at cooling time
    Lbol = (10.**(Lbol))*Lsun; #[erg/s] # get_lbol(coolingTime)

    # use Lbol from Johnstone+2020
    get_lbol(coolingTime/1e6, period_star)
    Lbol = float(np.loadtxt('temp_lbol.txt'))

    #lst=[]
    #for t in np.linspace(1,1e3,1000):
    #    get_lbol(t,period_star)
    #    lst.append(float(np.loadtxt('temp_lbol.txt')))
   #fn=interpolate.interp1d(Etrack[:,0],10**(Etrack[:,1])*Lsun)
   #plt.loglog(np.linspace(1e6,1e9,1000),fn(np.linspace(1e6,1e9,1000)))
   #plt.loglog(np.linspace(1,1e3,1000)*1e6,lst,label='johnstone')
   #plt.legend()
   #plt.show()
...
if do_evolve_planet:
    ...
    run_time = inlist.evolve_planet(mesh_delta_coeff, 1.e7, orb_sep, maxage, nameEtrack1, escape_model, t_sat, a_x, b_x, a_euv, b_euv, period_star, inlist7, relaxirradmodel, evolvedmodel)
    ...
    print("planet final radius @ "+str(maxage/1.e9)+" Gyr : "+str(last[22][0:6])+" Rearth.")
    print("planet final mass @ "+str(maxage/1.e9)+" Gyr : "+str(round(float(last[21]),3))+" Mearth.")
    print("% of initial atmosphere lost at "+str(maxage/1.e9)+" Gyr : "+str(round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2))+" %.")
    ...
    #f escape_model=='HD':
    #   dat=pd.read_pickle('mesa_HD_Johnstone.pkl')
    #   dat.loc[len(dat)]=[mcore/mearth,gcr,float(sys.argv[3]),period_star,last[22][0:6],last[22][0:6],round(float(last[21]),3),round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2)]
    #   print(dat)
    #   dat.to_pickle('mesa_HD_Johnstone.pkl')
    #elif escape_model=='EL':
    #    dat=pd.read_pickle('mesa_EL_Johnstone.pkl')
    #    dat.loc[len(dat)]=[mcore/mearth,gcr,float(sys.argv[3]),period_star,last[22][0:6],last[22][0:6],round(float(last[21]),3),round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2)]
    #    print(dat)
    #    dat.to_pickle('mesa_EL_Johnstone.pkl')
    #elif escape_model=='ELR':
    #    dat=pd.read_pickle('mesa_ELR_2Gyr.pkl')
    #    dat.loc[len(dat)]=[mcore/mearth,gcr,float(sys.argv[3]),period_star,last[22][0:6],last[22][0:6],round(float(last[21]),3),round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2)]
    #    print(dat)
    #    dat.to_pickle('mesa_ELR_2Gyr.pkl')
    #elif escape_model=='HD_highmass':
    #    dat=pd.read_pickle('mesa_HD_Johnstone.pkl')
    #    dat.loc[len(dat)]=[mcore/mearth,gcr,float(sys.argv[3]),period_star,last[22][0:6],last[22][0:6],round(float(last[21]),3),round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2)]
    #    print(dat)
    #    dat.to_pickle('mesa_HD_Johnstone.pkl')
    #if escape_model=='EL_Parker':
    #    dat=pd.read_pickle('mesa_ELP.pkl')
    #    dat.loc[len(dat)]=[mcore/mearth,gcr,float(sys.argv[3]),period_star,last[22][0:6],last[22][0:6],round(float(last[21]),3),round(np.abs(100.* ((mcore * gcr / mearth)-(float(last[21])-mcore/mearth))/(mcore * gcr / mearth)),2)]
    #    print(dat)
    #    dat.to_pickle('mesa_ELP.pkl')
...
if thermal_:
    ...
    run_time = inlist.evolve_planet(mesh_delta_coeff, 1.e7, orb_sep, maxage, nameEtrack1, escape_model, t_sat, a_x, b_x, a_euv, b_euv, period_star, inlist8, relaxirradmodel, evolvedmodel_thermal)
    ...
    #    dat['Rf_5Gyr']=dat['Rf_5Gyr'].replace(dat_0['Rf_5Gyr'],(last[22][0:6]))
    #    print(dat)
    #    dat.to_pickle('mesa_HD_Johnstone.pkl')
    #elif escape_model=='EL':
    #    dat=pd.read_pickle('mesa_EL_Johnstone.pkl')
